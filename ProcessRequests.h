    ...
    while (!m.get_requests().empty()) {
        ...
        if (request.get_type() == "Remove") {
            ...
            if (!error) {
                ...
                if (turmas.empty()) {
                    ...
                    ostringstream input;
                    input << "Success Removed Student: For input meca: " << meca << " class: " << turma << " Uc: " << uc << " Student successfully Removed";
                    log_text = LogMessage(input.str());
                    log.push_back(log_text);
                }
            }
        ...
        if (request.get_type() == "Add") {
            ...
            if (!(regex_search(balance, std::regex("[YN]{1}")))) {
                log_text = LogMessage(
                        std::string("Error Adding Student: Invalid Input balaced or not Input: " + balance));
                log.push_back(log_text);
                error = true;
            }
            ...
            if (!error) {
                ...
                if (!(it_stud == m.get_students().end())) { turmas = it_stud->get_turm(); }
                ...
                //if student was found
                    ...
                    if (CheckCap(m, temp_turma, n_cap)) {
                        ...
                        if (CheckBalance(m, uc) == false && balance == "N") {
                            ostringstream input;
                        ...
                        else if (CheckBalance(m, uc) == false && balance == "Y") {
                            ostringstream input;
                            input << "Note Adding Student: For input meca: " << meca << " class: " << turma << " Uc: " << uc << " Unbalance detected and choose to ignore it";
                            log_text = LogMessage(input.str());
                            log.push_back(log_text);
                        ...
                        else if ((CheckBalance(m, uc) == true && balance == "N") || (CheckBalance(m, uc) == true && balance == "Y")) {
                            if (CheckCompatability(m, it_stud->get_turm(), UcTurma(uc, turma)))
                            {
                                UcTurma aux_adding_uc = UcTurma(uc, turma);
                                list<UcTurma> aux_list = it_stud->get_turm();
                    ...
                    //if n_students has surpassed the CAP
                        ...
                        ostringstream input;
                        input << "Error Adding Student: For input meca: " << meca << " class: " << turma << " Uc: " << uc << " Can't add more because of Cap";
                        log_text = LogMessage(input.str());
                        log.push_back(log_text);
                    }
